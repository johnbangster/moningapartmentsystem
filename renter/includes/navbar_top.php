...
if (isset($_SESSION['auth_user'])) {
    ...
    if ($auth_role === 'admin' || $auth_role === 'employee') {

        // Admin/Employee see shared & personal notifications
        $count_sql = "
        ...
        // Renters see only their own notifications
        $count_sql = "
            SELECT COUNT(*) AS total 
            FROM notifications 
    ...
    // Execute unread count
    $count_result = mysqli_query($con, $count_sql);
    if ($count_row = mysqli_fetch_assoc($count_result)) {
        $unread_total = intval($count_row['total']);
    }
    ...
    <ul class="navbar-nav ms-auto me-md-0 me-3 me-lg-4">
        ...
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle" href="#" id="notifDropdown" role="button" 
            ...
            <span id="notifCount" class="badge bg-danger"></span>
          </a>
